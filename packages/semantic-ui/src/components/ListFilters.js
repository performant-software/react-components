...
import React, { useCallback, useEffect, useMemo, type Element } from "react";
import {
  Button,
  Checkbox,
  Dropdown,
...
const OperatorsByType = {
  [FilterTypes.boolean]: [FilterOperators.equal],
  [FilterTypes.date]: [FilterOperators.equal, FilterOperators.whereIn],
...
const ListFilters = (props: Props) => {
  ...
  const getOperatorsByType = useCallback((type: string) => {
    ...
    return _.filter(
      ...
      (option) => !operators || _.contains(operators, option.key)
    );
  }, []);
  ...
  const renderInput = useCallback(
    (filter: Filter) => {
      ...
      if (
        filter.operator === FilterOperators.empty ||
        filter.operator === FilterOperators.notEmpty
      ) {
        return null;
      }
      ...
      if (
        ...
        filter.type === FilterTypes.integer
      ) {
        return (
          <Input
            onChange={(e, { value }) =>
              props.onSaveChildAssociation("filters", { ...filter, value })
            }
            value={filter.value}
          />
        );
          ...
          <AssociatedDropdown
            ...
            onSelection={(item) => {
              props.onSaveChildAssociation("filters", {
                ...
                searchQuery:
                  renderSearchQuery &&
                  operator !== FilterOperators.whereIn &&
                  renderSearchQuery(item),
                value: operator !== FilterOperators.whereIn ? item.id : item,
              });
            }}
            renderOption={renderOption}
            required
            searchQuery={searchQuery || ""}
            value={value || (operator === FilterOperators.whereIn ? [] : null)}
            multiple={operator === FilterOperators.whereIn}
            key={operator}
          />
        );
      }
  ...
  const filters = useMemo(
    () => _.sortBy(props.filters, props.item.onSort || onDefaultSort),
          ...
          <Modal.Header>
            <Grid columns={2}>
              <Grid.Column verticalAlign="middle">
                <Header content={i18n.t("ListFilters.title")} />
              </Grid.Column>
              <Grid.Column textAlign="right">
                <DropdownButton
                  ...
                  onChange={(e, { value }) => {
                    ...
                    props.onSaveChildAssociation(
                      "filters",
                      props.onCreateFilter({
                        ...filter,
            ...
            {!_.isEmpty(props.item.filters) && (
              <Grid>
                {_.map(props.item.filters, (filter) => (
                  <Grid.Row columns={4} key={filter.key} verticalAlign="middle">
                    <Grid.Column>
                      <Header content={filter.label} />
                    </Grid.Column>
                    <Grid.Column width={5}>
                      <Dropdown
                    ...
                    <Grid.Column width={5}>{renderInput(filter)}</Grid.Column>
                    <Grid.Column width={1}>
                      <Button
                        basic
                        icon="times"
...
export { FilterTypes, FilterOperators, FilterOperatorOptions };
