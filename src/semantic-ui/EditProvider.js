...
class EditProvider extends Component<Props, State> {
  /**
   ...
   */
  componentDidMount() {
    if (this.props.onInitialize && this.state.item.id) {
      this.props.onInitialize(this.state.item.id).then((item) => this.setState({ item }));
    }
  }
